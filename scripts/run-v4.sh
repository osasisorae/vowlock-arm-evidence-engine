...
manifest="${V4_MANIFEST:-${root_dir}/experiment.v4.json}"
host_label="${V4_HOST_LABEL:-ubuntu-22.04-arm}"
work_dir="${root_dir}/work/v4"
source_dir="${work_dir}/llama.cpp"
build_dir="${work_dir}/build"
model_dir="${root_dir}/model"
result_root="${root_dir}/results/v4"
jobs="${BUILD_JOBS:-$(getconf _NPROCESSORS_ONLN)}"
llama_ref="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["runtime"]["commit"])' "${manifest}")"

arch="$(uname -m)"
if [[ "${arch}" != "aarch64" && "${arch}" != "arm64" ]]; then
  echo "Refusing Version 4 performance run: real Arm64 is required; detected ${arch}." >&2
  exit 2
fi
...
python3 "${root_dir}/v4_authority.py" --manifest "${manifest}" --root "${root_dir}" --output "${result_root}/raw/${host_label}/preflight-shell.json" >/dev/null
...
IFS=$'\t' read -r filename model_url model_sha model_bytes < <(python3 -c 'import json,sys; m=json.load(open(sys.argv[1]))["model"]; print("\t".join(str(m[k]) for k in ("filename","url","sha256","bytes")))' "${manifest}")
...
if [[ "${actual_bytes}" != "${model_bytes}" ]]; then
  echo "Size mismatch: ${actual_bytes} != ${model_bytes}" >&2
  exit 3
fi

python3 "${root_dir}/scripts/run-v4.py" \
  --manifest "${manifest}" \
